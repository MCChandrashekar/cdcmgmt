class TreeChart {
    ...
    initialize() {
        ...
        // Process alias data for display
        this.processAliasNodes(this.root);
    ...
    // Process data to structure alias nodes correctly
    processAliasNodes(rootNode) {
        if (!rootNode || !rootNode.data) return;
        
        const processNode = (node) => {
            if (node.data.active) {
                // Handle active zone group
                node.data.name = node.data.active.ZoneGrpName;
                node.data.id = node.data.active.ZoneGrpId;
                node.data.type = 'root';
                
                // Create zone children
                node.children = node.data.active.ZoneMembers.map(zone => ({
                    data: {
                        name: zone.ZoneName,
                        id: zone.ZoneId,
                        type: 'zone',
                    ...
                    // Create alias children for each zone
                    children: zone.AliasMembers.map(alias => ({
                        data: {
                ...
                // Add inactive zone group if it exists
                if (node.data.inactive) {
                    const inactiveZones = node.data.inactive.ZoneMembers.map(zone => ({
                        data: {
                            name: zone.ZoneName,
                            id: zone.ZoneId,
                            type: 'zone',
                    ...
                    // Add inactive zones to children array
                    if (inactiveZones.length > 0) {
                        node.children = [...node.children, ...inactiveZones];
                    }
                }
            }
    ...
    update(source) {
        ...
        const nodeEnter = node.enter().append('g')
            ...
            .on('mouseover', (event, d) => {
                ...
                if (d.data.type === 'root') {
                    ...
                    // Access alias data correctly
                    const type = d.data.Type || 'Unknown';
                    const ip = d.data.IPAddress || '';
                    const nqn = d.data.NQN || '';
                    
                    tooltipContent = `${type}: ${ip}<br>${nqn}`;
                }
        ...
        // Add detail nodes for aliases
        nodeEnter.filter(d => d.data.type === 'alias')
            .each((d, i, nodes) => {
                const node = d3.select(nodes[i]);
                const detailGroup = node.append('g')
                    .attr('class', 'alias-details')
                    .attr('transform', `translate(0, ${this.nodeHeight})`);
                
                // Add details rectangle
                ...
                const detailText = detailGroup.append('text')
                    ...
                    .attr('y', this.nodeHeight/2)
                    .attr('text-anchor', 'middle')
                    .attr('class', 'alias-detail-text');
                
                // First line: Type and IP
                detailText.append('tspan')
                    .attr('x', 0)
                    .attr('dy', '-0.2em')
                    .text(`${d.data.Type || 'Unknown'}: ${d.data.IPAddress || ''}`);
                
                // Second line: NQN (shortened if needed)
                let nqn = d.data.NQN || '';
                if (nqn.length > 20) {
                    nqn = nqn.substring(0, 17) + '...';
                }
