...
class TreeVisualization {
    ...
    updateLayout() {
        const treeLayout = d3.tree().nodeSize([this.nodeSpacing, this.width * 0.25]);

        // Generate and position host tree on the left
        ...
        // Generate and position storage tree on the right
        treeLayout(this.storageRoot);
        this.storageRoot.descendants().forEach(d => {
            d.x += this.height / 2;
            d.y = this.width - (d.depth * 180) - 50;
            // Store initial positions for animations
            d.x0 = d.x;
            d.y0 = d.y;
        });
