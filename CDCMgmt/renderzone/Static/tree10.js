 ...
 class TreeChart {
    ...
    initialize() {
        ...
        this.treeLayout = d3.tree()
            //.size([this.height/2, this.width - this.margin.left - this.margin.right])  // Modified sizing
            //.separation((a, b) => {
    ...
    update(source) {
        ...
        const nodeEnter = node.enter().append('g')
            ...
            .on('mouseover', (event, d) => {
                ...
                if (d.data.type === 'root') {
                    // Example: "ZoneGroup ID: 0, Zone Count: 5"
                    const zCount = d.children ? d.children.length : 0;
                    tooltipContent = `ZoneGroup ID: ${d.data.id}<br>Zone Count: ${zCount}`;
                } else if (d.data.type === 'zone') {
                    // Example: "Zone ID: 4, Alias Count: 3"
                    const aliasCount = d.children ? d.children.length : 0;
                    tooltipContent = `Zone ID: ${d.data.id}<br>Alias Count: ${aliasCount}`;
                } else if (d.data.type === 'alias') {
                    // Example: show host/storage, IP, NQN, etc.
                    // Suppose we have d.data.host, d.data.ip, d.data.nqn, etc.
                    tooltipContent = '';
                    if (d.data.host) {
                        tooltipContent += `Host/Storage: ${d.data.host}<br>`;
                    }
                    if (d.data.ip) {
                        tooltipContent += `IP: ${d.data.ip}<br>`;
                    }
                    if (d.data.nqn) {
                        tooltipContent += `NQN: ${d.data.nqn}<br>`;
                    }
                    if (!tooltipContent) {
                        tooltipContent = 'Alias node';
                    }
                } else {
                    tooltipContent = `ID: ${d.data.id || 'N/A'}`;
                }
        ...
        nodeEnter.append('rect')
            ...
            .attr('height', this.nodeHeight+20)
            .attr('x', -this.nodeWidth/2)
            .attr('y', -(this.nodeHeight/2) - 10)
            .attr('class', d => d.isCollapsed ? 'collapsed' : '');

        //Add Text in two lines
        const label = nodeEnter.append('text')
            .attr('text-anchor', 'middle')
            .attr('class', d => d.isCollapsed ? 'collapsed-text' : ''); // optional

        // First line: Type
        label.append('tspan')
            .attr('x', 0)
            .attr('dy', '-0.2em') // shift this line slightly up
            .text(d => {
                if (d.data.type === 'root') return 'Zone Group';
                if (d.data.type === 'zone') return 'Zone';
                if (d.data.type === 'alias') return 'Alias';
                return ''; 
            });

        // Second line: Actual Name
        label.append('tspan')
            .attr('x', 0)
            .attr('dy', '1.2em') // shift down from first line
            .text(d => truncateText(d.data.name, 12));
