...
// }
      ...
      initializeNodes(node) {
        ...
        node.descendants().forEach(d => {
            ...
            // Collapse everything except the root and group levels
            if (d.depth === 0) {
                // Root: Keep children (groups) visible
                d.children = d.children;
            } else if (d.depth === 1) {
                // Groups: Store children in _children and collapse
                if (d.children) {
                    d._children = d.children;
                    d.children = null;
                }
            } else if (d.depth > 1) {
                // Zones and aliases: same collapsing logic
                if (d.children) {
                    d._children = d.children;
                    d.children = null;
                }
    ...
    // Toggle expand/collapse for group or zone
    if (['group', 'zone'].includes(d.data.type)) {
        if (d.children) {
            d._children = d.children;
            d.children = null;
        } else {
            ...
            // Only collapse other zones if it's a zone node
            if (d.data.type === 'zone') {
                this.collapseOtherZones(d);
            }
        }
    ...
    // Handle collapsed summary node (e.g., "+3 more")
    else if (d.isCollapsed && d.collapsedNodes) {
        d.collapsedNodes.forEach(n => {
            n.children = n._children;
            n._children = null;
    ...
    update(source) {
        ...
        const nodeEnter = node.enter().append('g')
            ...
            if (d.data.type === 'root') {
                ...
                const zoneCount = d.children ? d.children.length : 0;
                tooltipContent = `ZoneGroup: ${d.data.name}<br>Zones: ${zoneCount}`;
                ...
                tooltipContent = `Alias: ${d.data.name}`;

                if (d.data.ip) {
                    tooltipContent += `: ${d.data.ip}`;
                }
                if (d.data.nqn) {
